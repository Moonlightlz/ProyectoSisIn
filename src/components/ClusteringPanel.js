...
const ClusteringPanel = () => {
  ...
  const { matrix: normMatrix } = zScoreNormalize(usedVectors);
  ...
  const renderResult = () => {
    ...
    const chartData = {
      datasets: Object.entries(clusterResult.clusters).map(([idx, items]) => ({
        ...
        data: items.map(item => ({
          x: item.mat?.stock || 0,
          y: item.mat?.lowStockThreshold || 0
        })),
    ...
    Object.entries(clusterResult.clusters).forEach(([idx, items]) => {
      datasets.push({
        label: clusterName(Number(idx)),
        data: items.map(item => ({
          x: item.mat?.stock || 0,
    ...
    return (
      <div className="chart-card clustering-card">
        ...
        <p style={{ marginTop: 4, color: '#64748b', fontSize: '0.85rem' }}>
          ...
          {source === 'sales' 
            ? "Mueva el cursor sobre los puntos para ver los valores reales de monto y cantidad." 
            : "Mueva el cursor sobre los puntos para ver la cantidad actual en almacén y el punto donde se debe reordenar (cantidad mínima permitida)."}
        </p>

        <div style={{ marginTop: 12, marginBottom: 16 }}>
          <button className="btn btn-secondary btn-sm" onClick={exportCSV}>Exportar CSV</button>
          <button className="btn btn-secondary btn-sm" style={{ marginLeft: 8 }} onClick={() => { setClusterResult(null); }}>Cerrar</button>
        </div>

        <table className="worker-hours-table" style={{ fontSize: '0.9rem' }}>
          <thead>
            <tr>
              <th>Segmento</th>
              <th>Tamaño</th>
              <th>Promedio Monto</th>
              <th>Promedio Cantidad</th>
              <th>Ejemplos</th>
            </tr>
          </thead>
          <tbody>
            {Object.keys(clusterResult.clusters)
              .map(lab => ({
                lab,
                items: clusterResult.clusters[lab],
                size: clusterResult.clusters[lab].length
              }))
              ...
              let avgQty = 0;
              if (source === 'sales') {
                avgAmount = items.reduce((s, it) => s + (it.totalAmount || (it.products ? it.products.reduce((a,p)=>a+(p.subtotal||0),0):0)), 0) / Math.max(1, size);
                avgQty = items.reduce((s, it) => s + (it.totalQuantity || (it.products ? it.products.reduce((a,p)=>a+(p.quantity||0),0):0)), 0) / Math.max(1, size);
              } else {
                avgAmount = items.reduce((s, it) => s + (it.mat?.stock || 0), 0) / Math.max(1, size);
                avgQty = items.reduce((s, it) => s + (it.movements?.length || 0), 0) / Math.max(1, size);
              }
              const examples = items.slice(0,3).map(it => source === 'sales' ? (it.distributor?.name || 'Sin distribuidor') : (it.mat?.supplier || 'Sin proveedor')).join(', ');
              return (
                <tr key={lab}>
                  <td>
                    {clusterName(Number(lab))}
                  </td>
                  <td>{size}</td>
                  <td>{avgAmount.toFixed ? avgAmount.toFixed(2) : avgAmount}</td>
                  <td>{avgQty.toFixed ? avgQty.toFixed(2) : avgQty}</td>
                  <td>{items.slice(0,3).map(it => source === 'sales' ? 
                    (it.distributor?.name || 'Sin distribuidor') : 
                    (it.mat?.supplier || 'Sin proveedor')).join(', ')}</td>
                </tr>
              );
            })}
          </tbody>
  ...
  return (
    <div className="chart-card clustering-card">
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 12 }}>
        ...
        <label>K:</label>
        <input type="number" min="2" max="10" value={k} onChange={e => setK(Number(e.target.value || 2))} style={{ width: 80 }} />
        <button className="btn btn-primary btn-sm" onClick={onRun} disabled={running}>{running ? 'Procesando...' : 'Ejecutar'}</button>
      </div>
        ...
        {source === 'sales' ? (
          <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
            <div>
              ...
              <br />
              <select value={selectedProviders} onChange={e => setSelectedProviders(e.target.value)} style={{ minWidth: 200 }}>
            ...
            <div>
              ...
              <br />
              <select value={selectedProducts} onChange={e => setSelectedProducts(e.target.value)} style={{ minWidth: 260 }}>
                <option value="">Todos</option>
            ...
            <div>
              <label>Tallas</label>
              <br />
              <select value={selectedSizes} onChange={e => setSelectedSizes(e.target.value)} style={{ minWidth: 120 }}>
                <option value="">Todos</option>
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
            <div>
              <label>Proveedores</label>
              <br />
              <select value={selectedSuppliers} onChange={e => setSelectedSuppliers(e.target.value)} style={{ minWidth: 220 }}>
                <option value="">Todos</option>
            ...
            <div>
              ...
              <br />
              <select value={selectedCategories} onChange={e => setSelectedCategories(e.target.value)} style={{ minWidth: 220 }}>
                <option value="">Todos</option>
